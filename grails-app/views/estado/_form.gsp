<%@ page import="com.ideasynegocios.general.Estado" %>
...
<div class="fieldcontain ${hasErrors(bean: estadoInstance, field: 'abreviatura', 'error')} ">
	<label for="abreviatura">
		<g:message code="estado.abreviatura.label" default="Abreviatura" />
		
	</label>
	<g:textField name="abreviatura" value="${estadoInstance?.abreviatura}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: estadoInstance, field: 'pais', 'error')} required">
	...
	<g:select id="pais" name="pais.id" from="${com.ideasynegocios.general.Pais.list()}" optionKey="id" required="" value="${estadoInstance?.pais?.id}" class="many-to-one"/>
</div>
